...
bool term_creation()
{
  ...
  std::shared_ptr<BoolectorTerm> bx = make_shared<BoolectorTerm>(btor, x,
                                                                 std::vector<shared_ptr<AbsTerm>> {},
                                                                 VAR);

  std::shared_ptr<BoolectorTerm> by = make_shared<BoolectorTerm>(btor, y,
                                                                 std::vector<shared_ptr<AbsTerm>> {},
                                                                 VAR);
  std::shared_ptr<BoolectorTerm> bx_ule_y = make_shared<BoolectorTerm>(btor, x_ulte_y,
                                                                       std::vector<shared_ptr<AbsTerm>> {bx, by},
                                                                       BVULE);
  ...
  //      can't delete the btor object because need it in BoolectorTerm destructors
  return res;
}
