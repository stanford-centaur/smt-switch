...
Term BoolectorSolver::make_term(bool b) const
{
  if (b)
  {
    return make_my_ptr<BoolectorTerm>(btor, boolector_const(btor, "1"));
  }
  else
  {
    return make_my_ptr<BoolectorTerm>(btor, boolector_const(btor, "0"));
  }
}
...
Term BoolectorSolver::make_term(int64_t i, const Sort & sort) const
{
  try
  {
    my_ptr<BoolectorSortBase> bs =
        std::static_pointer_cast<BoolectorSortBase>(sort);
    // note: give the constant value a null PrimOp
    return make_my_ptr<BoolectorTerm>(btor, boolector_int(btor, i, bs->sort));
  }
...
Term BoolectorSolver::make_term(const Term & val, const Sort & sort) const
{
  ...
  {
    my_ptr<BoolectorTerm> bt = std::static_pointer_cast<BoolectorTerm>(val);
    my_ptr<BoolectorSortBase> bs =
        std::static_pointer_cast<BoolectorSortBase>(sort);
    return make_my_ptr<BoolectorTerm>(
...
{
  my_ptr<BoolectorTerm> bt = std::static_pointer_cast<BoolectorTerm>(t);
...
Term BoolectorSolver::get_value(const Term & t) const
{
  ...
  if ((sk == BV) || (sk == BOOL))
  {
    ...
    // note: give the constant value a null PrimOp
    result = make_my_ptr<BoolectorTerm>(btor, bc);
  }
  else if (sk == ARRAY)
  {
    my_ptr<BoolectorSortBase> bs =
    ...
    my_ptr<BoolectorSortBase> b_elemsort =
    ...
    result = make_my_ptr<BoolectorTerm>(btor, stores);
                                                   ...
                                                   Term & out_const_base) const
{
  ...
  my_ptr<BoolectorTerm> barr = std::static_pointer_cast<BoolectorTerm>(arr);
  ...
  for (uint32_t i = 0; i < size; i++)
  {
    if (std::string(bindices[i]) == "*")
    {
      ...
      out_const_base = make_my_ptr<BoolectorTerm>(btor, belem);
    }
    else
    {
      ...
      Term idx = make_my_ptr<BoolectorTerm>(btor, bidx);
      Term val = make_my_ptr<BoolectorTerm>(btor, belem);
...
void BoolectorSolver::get_unsat_core(UnorderedTermSet & out)
{
  ...
  while (*bcore)
  {
    out.insert(make_my_ptr<BoolectorTerm>(btor, boolector_copy(btor, *bcore)));
    ++bcore;
  }
}
...
Sort BoolectorSolver::make_sort(SortKind sk) const
{
  if (sk == BOOL)
  {
    return make_my_ptr<BoolectorBVSort>(btor, boolector_bool_sort(btor), 1);
  }
  else
  {
...
Sort BoolectorSolver::make_sort(SortKind sk, uint64_t size) const
{
  if (sk == BV)
  {
    return make_my_ptr<BoolectorBVSort>(
...
Sort BoolectorSolver::make_sort(SortKind sk, const SortVec & sorts) const
{
  if (sk == FUNCTION)
  {
    ...
    my_ptr<BoolectorSortBase> btor_return_sort =
    ...
    return make_my_ptr<BoolectorUFSort>(btor, btor_fun_sort, sorts, returnsort);
  }
...
Term BoolectorSolver::make_symbol(const std::string name, const Sort & sort)
{
  ...
  Term term = make_my_ptr<BoolectorTerm>(btor, n);
...
Term BoolectorSolver::make_param(const std::string name, const Sort & sort)
{
  ...
  return make_my_ptr<BoolectorTerm>(btor, n);
}
...
Term BoolectorSolver::make_term(Op op, const Term & t) const
{
  ...
  else
  {
    ...
    return make_my_ptr<BoolectorTerm>(btor, btor_res);
  }
}
    ...
    const Term term, const UnorderedTermMap & substitution_map) const
{
  ...
  my_ptr<BoolectorTerm> bt = std::static_pointer_cast<BoolectorTerm>(term);

  my_ptr<BoolectorTerm> key;
  my_ptr<BoolectorTerm> value;
  ...
  return make_my_ptr<BoolectorTerm>(btor, substituted);
}
...
Term BoolectorSolver::apply_prim_op(PrimOp op, Term t) const
{
  try
  {
    ...
    return make_my_ptr<BoolectorTerm>(btor, result);
  }
...
Term BoolectorSolver::apply_prim_op(PrimOp op, Term t0, Term t1) const
{
  try
  {
    my_ptr<BoolectorTerm> bt0 = std::static_pointer_cast<BoolectorTerm>(t0);
    my_ptr<BoolectorTerm> bt1 = std::static_pointer_cast<BoolectorTerm>(t1);

    BoolectorNode * result;
    if (op == Apply)
    {
      my_ptr<BoolectorTerm> bt = std::static_pointer_cast<BoolectorTerm>(t1);
...
Term BoolectorSolver::apply_prim_op(PrimOp op, Term t0, Term t1, Term t2) const
{
  try
  {
    my_ptr<BoolectorTerm> bt0 = std::static_pointer_cast<BoolectorTerm>(t0);
    my_ptr<BoolectorTerm> bt1 = std::static_pointer_cast<BoolectorTerm>(t1);
    my_ptr<BoolectorTerm> bt2 = std::static_pointer_cast<BoolectorTerm>(t2);
    BoolectorNode * result;
    if (op == Apply)
    {
      my_ptr<BoolectorTerm> bt1 = std::static_pointer_cast<BoolectorTerm>(t1);
