 ...
 * One example of overriding visit_term to customize the behavior of the walker. */
class IndicatorTreeWalker : public TreeWalker {
  ...
  /* builds up map assigning fresh boolean variables to each occurrence of each term in the formula, where the occurrences
   * are represented as a  pair containing the formula the term appears in and the path indicating the term's location in the
   * formula it appears in.
   * For example, the formula x+2=y traversed with this custom version of visit_term will build the corresponding map containing:
   ...
   * As in this example, each occurrence of each term in the formula should correspond to a fresh variable mapping to its occurrence in the formula
   */
  TreeWalkerStepResult visit_term(smt::Term & formula, smt::Term & term, std::vector<int> & path) override{

    Sort boolsort = solver_->make_sort(BOOL);

    /* for each node visited, create a fresh boolean variable, bn, to map to an occurrence of a term in the formula (the formula in which
     * it occurs and the path indicating the term's location in the formula)
     */
    ...
    //for each node visited in path traversal, saves in cache mapping from fresh boolean variable to the pair giving the full formula in which it occurs and the path indicating its place in the formula
    save_in_cache(b, occ);
...
{
  // traversal of simple formula using TreeWalker, test that cache gets built up as expected
...
TEST_P(UnitWalkerTests, PathDecomp)
{
  ...
  if (sc.is_logging_solver) {
    ...
    // check that the cache populated by TreeWalker is equivalent to the expected map
    for (auto const& x : UndMap){
      EXPECT_EQ(x.second.first, expected_map[x.first].first);
      ASSERT_EQ(x.second.second.size(), expected_map[x.first].second.size());
...
{
  // test traversal using TreeWalker of formula with Uninterpreted Functions that can more than one argument
...
TEST_P(UnitWalkerTests, FreshVars){
  ...
  if (sc.is_logging_solver) {
    ...
    // manually check equivalency of cache populated by IndicatorTreeWalker with the expected map
    // expected_path variable to check against expected path for each node
