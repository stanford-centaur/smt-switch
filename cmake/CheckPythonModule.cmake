...
function(check_python_module module)
  ...
  execute_process(
    COMMAND
      ${Python_EXECUTABLE} -c
      "import ${module}; print(${module}.__version__ if hasattr(${module}, '__version__') else '')"
    RESULT_VARIABLE _${module}_version_status
    OUTPUT_VARIABLE _${module}_version
    ERROR_VARIABLE _${module}_version_error
    ERROR_STRIP_TRAILING_WHITESPACE
    OUTPUT_STRIP_TRAILING_WHITESPACE
  )
  ...
  if(_${module}_version_status EQUAL 0 AND _${module}_version)
    ...
    if(min_version)
      ...
      execute_process(
        COMMAND
          ${Python_EXECUTABLE} -c
          "import packaging.version; exit(0 if packaging.version.parse('${_${module}_version}') >= packaging.version.parse('${min_version}') else 1)"
        RESULT_VARIABLE _${module}_min_version_check
        ERROR_VARIABLE _${module}_min_version_error
        ERROR_STRIP_TRAILING_WHITESPACE
      )
      ...
      if(NOT _${module}_min_version_check EQUAL 0)
        ...
        message(
          STATUS
          "Found ${module} version ${_${module}_version} (required: >= ${min_version})"
        )
      endif()
    else()
  ...
  execute_process(
    COMMAND
      ${Python_EXECUTABLE} -c
      "import ${module}, os; print(os.path.dirname(${module}.__file__) if hasattr(${module}, '__file__') else '')"
    RESULT_VARIABLE _${module}_path_status
    OUTPUT_VARIABLE _${module}_path
    ERROR_VARIABLE _${module}_path_error
    ERROR_STRIP_TRAILING_WHITESPACE
    OUTPUT_STRIP_TRAILING_WHITESPACE
  )
