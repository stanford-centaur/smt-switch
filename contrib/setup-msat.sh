...
if [ ! -d "$DEPS/mathsat" ]; then
    ...
    if [[ "$OSTYPE" == linux* ]]; then
        wget -O mathsat.tar.gz mathsat.fbk.eu/download.php?file=mathsat-5.6.1-linux-x86_64.tar.gz 
    elif [[ "$OSTYPE" == darwin* ]]; then
        wget -O mathsat.tar.gz mathsat.fbk.eu/download.php?file=mathsat-5.6.1-darwin-libcxx-x86_64.tar.gz
    elif [[ "$OSTYPE" == msys* ]]; then
        wget -O mathsat.tar.gz mathsat.fbk.eu/download.php?file=mathsat-5.6.1-win64-msvc.zip
