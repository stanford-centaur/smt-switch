...
void BoolectorSolver::set_opt(const std::string option, const std::string value) const
  {
    throw NotImplementedException("String value for set_opt unimplemented");
  }

Sort BoolectorSolver::declare_sort(const std::string name, unsigned int arity) const
  {
...
Sort BoolectorSolver::make_sort(Kind k) const
  {
    if (k == BOOL)
    {
...
Sort BoolectorSolver::make_sort(Kind k, unsigned int size) const
  {
    if (k == BV)
    {
      Sort s(
...
Sort BoolectorSolver::make_sort(Kind k, Sort idxsort, Sort elemsort) const
  {
    if (k == ARRAY)
    {
...
Sort BoolectorSolver::make_sort(Kind k, std::vector<Sort> sorts, Sort sort) const
  {
    if (k == UNINTERPRETED)
    {
